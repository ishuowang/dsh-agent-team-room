...
export function apply(ctx) {
    ctx.tools.register(defineTool({
        name: 'room_create',
        description: 'Create a persistent Room owned by this Session. Room contains no built-in roles, scenarios, prompts, or skills.',
        parameters: {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Attach an existing continuable direct-child DSH Session to an owned Room. This does not create a role, '
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Detach a member and optionally interrupt its current turn. The backing Session or external member is not deleted.',
        parameters: {
            room_id: { type: 'string', required: true, description: 'Open Room id.' },
        ...
        async execute(args, exec) {
            return jsonValue({
                member: await ctx.rooms.removeMember(callingAgent(exec), args.room_id, args.member_id, args.interrupt_running ?? true),
            });
        },
    }));
    ctx.tools.register(defineTool({
        ...
        output: jsonOutput,
        async execute(args, exec) {
            return jsonValue(await ctx.rooms.sendMessage(callingAgent(exec), args.room_id, args.to_member_id, args.message, exec.signal));
        },
    }));
    ctx.tools.register(defineTool({
        name: 'room_broadcast',
        description: 'Broadcast one message through every active member provider. Per-member failures remain explicit and message '
    ...
    ctx.tools.register(defineTool({
        name: 'room_close',
        description: 'Close an owned Room and optionally interrupt active members. Backing Sessions remain independently available.',
        parameters: {
            room_id: { type: 'string', required: true, description: 'Open Room id.' },

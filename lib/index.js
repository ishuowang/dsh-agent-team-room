...
import { getRoomTemplate, listRoomTemplates, } from './templates.js';
import { ROOM_SCHEMA_VERSION, isTerminalTask, roomSummary, } from './types.js';
...
export default class RoomRuntime extends Service {
     ...
     */
    async createRoomFromTemplate(parent, input, signal) {
        ...
        if (requiredMembers > this.config.maxMembersPerRoom) {
            throw new Error(`agent-team-room: template ${template.id} requires ${requiredMembers} room members, `
    ...
    listAllRooms(includeClosed = true) {
        return [...this.roomsById.values()]
    ...
    getTask(parent, roomId, taskId) {
        const room = this.ownedRoom(parent, roomId);
        const task = room.tasks.find(candidate => candidate.id === taskId);
        if (!task)
            throw new Error(`agent-team-room: unknown task ${taskId}`);
        return structuredClone(task);
    }
    async addAgent(parent, roomId, input, signal) {
        ...
        if (agentId) {
            ...
            const options = {};
            if (input.modelProvider?.trim())
                options.provider = input.modelProvider.trim();
            if (input.model?.trim())
    ...
    async sendMessage(parent, roomId, targetAgentId, message, signal) {
        const room = this.openOwnedRoom(parent, roomId);
    ...
    async broadcast(parent, roomId, message, signal) {
        ...
        if (members.length === 0)
            throw new Error('agent-team-room: room has no Agent members');
    ...
    async assignTask(parent, roomId, input, signal) {
        const room = this.openOwnedRoom(parent, roomId);
        ...
        await this.changed(room);
        try {
            task.messageId = await this.followup(parent, member.agentId, `[Room task ${task.id}]\nRoom id: ${room.id}\nTitle: ${task.title}\n\n${task.instructions}`
                + `\n\nWhen finished, call room_task_complete with room_id "${room.id}", task_id "${task.id}", `
                + 'status "completed", and a concise report. If blocked, call it with status "failed" and explain why.', signal);
            task.status = 'running';
            task.updatedAt = now();
            member.status = 'working';
    ...
    async completeTask(reporter, roomId, taskId, input) {
        ...
        if (input.status === 'completed')
            ...
            task.error = report;
        if (member.activeTaskId === task.id)
            delete member.activeTaskId;
        member.lastResult = report;
        member.status = input.status === 'completed' ? 'idle' : 'error';
        member.updatedAt = timestamp;
        this.appendEvent(room, input.status === 'completed' ? 'task.completed' : 'task.failed', input.status === 'completed'
            ? `${member.name} completed “${task.title}”`
            : `${member.name} reported “${task.title}” failed`, { actorAgentId: reporter.id, targetAgentId: reporter.id, taskId: task.id });
        await this.changed(room);
        return structuredClone(task);
    }
    async removeAgent(parent, roomId, agentId, interruptRunning = true) {
        const room = this.openOwnedRoom(parent, roomId);
    ...
    async closeRoom(parent, roomId, input) {
        ...
        for (const member of room.members) {
            if (member.kind !== 'agent' || member.status === 'removed')
                continue;
            if (shouldInterrupt) {
                this.ctx.subagents.interrupt(SessionId(member.agentId), { kind: 'ancestor', agent: parent });
    ...
    async waitForTasks(parent, roomId, taskIds, timeoutMs, signal) {
        ...
        for (const taskId of selected) {
            if (!initial.tasks.some(task => task.id === taskId))
                throw new Error(`agent-team-room: unknown task ${taskId}`);
        }
        const boundedTimeout = Math.max(0, Math.min(Math.trunc(timeoutMs), 300_000));
        const snapshot = () => {
            const room = this.ownedRoom(parent, roomId);
            const tasks = selected.map(taskId => room.tasks.find(task => task.id === taskId));
            return {
    ...
    async followup(parent, agentId, text, signal) {
        const messageId = await this.ctx.subagents.followup(parent, SessionId(agentId), [{ type: 'text', text }], {
    ...
    ownedRoom(parent, roomId) {
        const room = this.room(roomId);
        if (room.leaderAgentId !== parent.id)
    ...
    activeAgentMember(room, agentId) {
        const member = room.members.find(candidate => candidate.agentId === agentId && candidate.kind === 'agent');
        if (!member || member.status === 'removed')
            throw new Error(`agent-team-room: agent ${agentId} is not an active room member`);
        return member;
    }
    assertMemberCapacity(room) {
    ...
    cancelMemberTask(room, member, reason) {
        if (!member.activeTaskId)
            return;

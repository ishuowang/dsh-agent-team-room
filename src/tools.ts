...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description:
      'List built-in Agent Team Room scenario templates, their orchestration shape, roles, and human approval '
      + 'gates. Listing does not create a room or start Agent Sessions.',
    parameters: {},
    output: jsonOutput,
    execute() {
      return Promise.resolve(jsonValue({
        templates: ctx.rooms.listRoomTemplates().map(template => ({
          id: template.id,
          name: template.name,
          description: template.description,
          category: template.category,
          orchestration: template.orchestration,
          experimental: template.experimental ?? false,
          roleCount: template.roles.length,
          roles: template.roles.map(role => ({ id: role.id, name: role.name, role: role.role })),
  ...
  ctx.tools.register(defineTool({
    name: 'room_create_from_template',
    description:
      'Create a durable room from a built-in scenario template and start every role as an independent continuable '
      + 'child Session. This can start several Agents and consume model quota. A partial provisioning failure closes '
      + 'the traceable room instead of deleting Sessions that were already created.',
    parameters: {
      template_id: { type: 'string', required: true, description: 'Stable id returned by room_template_list.' },
      name: { type: 'string', description: 'Optional room name override.' },
      objective: { type: 'string', description: 'Optional shared objective override.' },
      provider: { type: 'string', description: 'DSH subagent provider override for every new member.' },
      model_provider: { type: 'string', description: 'LLM provider override for every new member.' },
      model: { type: 'string', description: 'LLM model override for every new member.' },
    },
    output: jsonOutput,
    async execute(args, exec) {
      return jsonValue(await ctx.rooms.createRoomFromTemplate(callingAgent(exec), {
  ...
  ctx.tools.register(defineTool({
    name: 'room_create',
    description:
      'Create a persistent Agent Team Room owned by this Agent. A room coordinates independent child Sessions; '
  ...
  ctx.tools.register(defineTool({
    name: 'room_add_agent',
    description:
      'Add an independent continuable Agent Session to an owned room. Omit agent_id to create a new child, or '
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Remove an Agent from an owned room and optionally interrupt its current turn. The independent DSH Session '
    ...
    output: jsonOutput,
    async execute(args, exec) {
      const member = await ctx.rooms.removeAgent(
        callingAgent(exec),
        args.room_id,
        args.agent_id,
  ...
  ctx.tools.register(defineTool({
    name: 'room_send',
    description:
      'Send a direct follow-up to one Agent in an owned room. It joins that Agent’s independent FIFO inbox and '
  ...
  ctx.tools.register(defineTool({
    name: 'room_broadcast',
    description:
      'Broadcast one follow-up to every active Agent member in an owned room. Returns per-Agent delivery results '
  ...
  ctx.tools.register(defineTool({
    name: 'room_assign',
    description:
      'Create a tracked room task and deliver it to one member Agent. A member can own only one non-terminal room '
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      'Report the terminal result of a tracked room task. Only the assigned Agent may report it. This explicit '
    ...
    parameters: {
      ...
      status: {
        type: 'string',
        required: true,
        enum: ['completed', 'failed'] as const,
  ...
  ctx.tools.register(defineTool({
    name: 'room_wait',
    description:
      'Wait until selected room tasks reach a terminal state or the timeout expires. Without task_ids, waits for '
    ...
    timeoutMs: 305_000,
    async execute(args, exec) {
      return jsonValue(await ctx.rooms.waitForTasks(
        callingAgent(exec),
        args.room_id,
        args.task_ids,
  ...
  ctx.tools.register(defineTool({
    name: 'room_close',
    description:
      'Close an owned room, cancel its non-terminal tracked tasks, and by default interrupt current member turns. '

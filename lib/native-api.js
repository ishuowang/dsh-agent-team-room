...
import { projectRoomConversation } from './conversation.js';
import { DSH_SESSION_MEMBER_PROTOCOL, DSH_SESSION_MEMBER_PROVIDER, } from './types.js';
...
export function nativeActivityLabel(room, event) {
    switch (event.type) {
        ...
        case 'message.direct': {
            if (!event.relay)
                return 'Legacy direct-message delivery';
        ...
        case 'message.broadcast': {
            if (!event.relay)
                return 'Legacy broadcast delivery';
...
async function memberEvents(ctx, member) {
...
function liveConversationRevision(ctx, room) {
...
export async function nativeRoomConversation(ctx, room) {
    ...
    await Promise.all(Array.from({
        ...
        while (cursor < members.length) {
            ...
            try {
                ...
                if (events)
                    histories.set(member.memberId, events);
                else
    ...
    return projectRoomConversation(room, histories, members.filter(member => unavailable.has(member.memberId)).map(member => member.memberId));
}
/**
...
export function apply(ctx) {
    ...
    const cachedConversation = (room) => {
        ...
        if (cached && cached.sourceRevision === sourceRevision && cached.expiresAt > currentTime)
            return cached.value;
        ...
        cachedEntry.timer = setTimeout(() => {
            if (conversationCache.get(room.id) === cachedEntry)
                conversationCache.delete(room.id);
    ...
    ctx.effect(() => {
        ...
        return () => {
            disposeRoute();
            for (const roomId of [...conversationCache.keys()])
                removeCached(roomId);
        };
